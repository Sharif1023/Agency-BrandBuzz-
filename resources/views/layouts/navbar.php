...
<header class="site-header-fixed bg-white border-b border-gray-100/70" style="position: fixed !important; top: 0 !important; left: 0 !important; right: 0 !important; width: 100% !important; z-index: 9999 !important;">
  ...
  <nav id="mobile-navigation" data-mobile-menu class="lg:hidden container-page pb-6" aria-label="Mobile navigation" hidden>
    <div class="grid grid-cols-2 gap-2">
      <?php foreach($nav as $file=>$label): ?><a class="nav-link px-3 <?= ($active ?? '') === $file ? 'active' : '' ?>" href="<?= e(page_url($file)) ?>"><?= e($label) ?></a><?php endforeach ?>
    </div>
  </nav>

  <noscript>
    <nav class="container-page flex flex-wrap gap-4 lg:hidden pb-5" aria-label="Navigation without JavaScript">
      <?php foreach($nav as $file=>$label): ?><a href="<?= e(page_url($file)) ?>"><?= e($label) ?></a><?php endforeach ?>
    </nav>
  </noscript>
</header>

<!-- হেডার ফিক্সড হওয়ার কারণে নিচের কন্টেন্ট যেন ভেতরে না ঢুকে যায় -->
<div class="h-24 w-full" aria-hidden="true"></div>